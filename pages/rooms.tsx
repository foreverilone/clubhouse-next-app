...
// из-за useEffect происходит задержка перед загрузкой данных, чтобы избавиться нужно использовать next методы
// в useEffect нельзя просто поместить async/await, поэтому используется такой способ:
  // React.useEffect(() => {
  //   (async() => {
  //     const { data } = await Axios.get('rooms.json'); 
  //     setRooms(data);
